      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        ...
        {jobs.length > 0 && (
          ...
          {Array.from({length:Math.ceil(jobs.length/6)}).map((_,index)=>(
            <a href="#job-list">
              <button onClick={()=> setCurrentPage(index+1)} className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded cursor-pointer ${currentPage === index + 1 ? 'bg-blue-400': 'text-gray-500'}`}>{index + 1}</button>
            </a>
          ))}
           <a href="#job-list">
